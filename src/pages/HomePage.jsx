  ...
  return (
    <div>
      ...
      <section
        ...
        style={{ backgroundImage: `url(${titleimge})`, height: "250px" }}
      >
        <div className="flex flex-col items-center justify-center h-full space-y-8 bg-black/30">
          <h1 className="text-white font-semibold text-2xl md:text-3xl">
            {t("herotext.headtext")}
          </h1>
          <p className="text-gray-100 text-xl font-medium max-w-screen-md mx-auto text-center px-4">
            {t("herotext.subtext")}
          </p>
        </div>
      </section>
      ...
      <section className="py-12 px-4">
        <div className="max-w-screen-xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 custom:grid-cols-4 gap-6">
            {cardData.map((card, index) => (
              <Link
                key={index}
                className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300"
              >
                <div className="p-6 flex flex-col items-center justify-between h-[400px]">
                  <img src={card.image} alt="" className=""/>
                  <h3 className="text-xl font-bold text-red-600/80 text-center">
                    {card.heading}
                  </h3>
                  <p className="text-gray-600 mb-4 text-center">{card.subheading}</p>
